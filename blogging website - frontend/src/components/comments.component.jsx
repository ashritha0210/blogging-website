...
//   await axios.post(import.meta.env.VITE_SERVER_DOMAIN + "/get-blog-comments", { blog_id, skip })
//     .then(({ data }) => {
//       // Add a property to each comment to track its level
//       data.forEach(comment => {
//         comment.childrenLevel = 0;
//       });

//       // Update the total number of loaded comments
//       setParentCommentCountFun(preVal => preVal + data.length);

//       // Merge the new comments with existing ones if provided
//       res = comment_array == null
//         ? { results: data }
...
//     // Update the blog's comment data
//     setBlog(prevBlog => ({
//       ...prevBlog,
...
//   return (
//     <div className={`max-sm:w-full fixed ${commentsWrapper ? "top-0 sm:right-0" : "top-[100%] sm:right-[-100%]"} duration-700 max-sm:right-0 sm:top-0 w-[30%] min-w-[350px] h-full z-50 bg-white shadow-2xl p-8px-16 overflow-y-auto overflow-x-hidden`}>
//       <div className="relative">
//         <h1 className="text-xl font-medium">Comments</h1>
//         <p className="text-lg mt-2 w-[70%] text-dark-grey line-clamp-1">{title}</p>
//         <button
...
//         </button>
//       </div>
//       <hr className="border-grey my-8 w-[100%] -ml-10" />
