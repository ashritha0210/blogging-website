...
import serviceAccountKey from "./blog-f995f-firebase-adminsdk-zxz6z-9d51b2b553.json" assert { type: "json" };
...
server.post("/google-auth", async (req, res) => {
   ...
   .then(async (decodedUser) => {
    ...
    if(user){
       if(!user.google_auth){
        return res.status(403).json({"error": "This email was signed up without google.Please log in with password to access to account"})
       } 
    }
    else
    {
